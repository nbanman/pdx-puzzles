...
use crate::{enums::intercardinals::Intercardinal, structs::coord::{Coord, Coord2U}};
...
impl<T, const N: usize> Grid<T, N> { 
    ...
    pub fn last_coord(&self) -> Coord<usize, N> {
        (self.data.len() - 1)
            .as_coord(self)
            .expect("Empty backing data should result in GridError, so len() - 1 should always be valid.")
    }
    ...
    pub fn coords(&self) -> impl Iterator<Item = Coord<usize, N>> + '_ {
        (0..self.len()).map(|index| {
            index
                .as_coord(self)
                .expect("Iterates through confirmed indices, so there should always be a valid Coord.")
        })
    }
}
...
impl<T> Grid<T, 2> {
    ...
    pub fn row(&self, row_index: usize) -> Result<impl Iterator<Item = &T> + '_, GridError> {
        if row_index >= self.height() { return Err(GridError::OutOfRange) }
    ...
    pub fn column(&self, column_index: usize) -> Result<impl Iterator<Item = &T> + '_, GridError> {
        if column_index >= self.width() { return Err(GridError::OutOfRange) }                
        let iterator = (0..self.height())
            .map(move |row| &self.data[row * self.width() + column_index]);
        Ok(iterator)
    }
    ...
    fn single_adjacent(&self, pos: Coord2U, dir: Intercardinal) -> Option<GridAdjacent<T>> {
        ...
        Some(GridAdjacent { index, pos, dir, value })
    }
...
mod tests {
    ...
    fn columns() {
        ...
        assert_eq!(
            vec![&2, &5, &8],
            np.column(1).unwrap().collect::<Vec<_>>()
        );
        assert_eq!(
            Err(GridError::OutOfRange),
    ...
    fn adjacent() {
        ...
        let test = vec![
            GridAdjacent { index: 1, pos: Coord([1, 0]), dir: Intercardinal::East, value: &2 }, 
            GridAdjacent { index: 4, pos: Coord([1, 1]), dir: Intercardinal::Southeast, value: &5 }, 
            GridAdjacent { index: 3, pos: Coord([0, 1]), dir: Intercardinal::South, value: &4 }
        ];
        assert_eq!(test, adjacent);

        // middle, diagonals
        let adjacent: Vec<_> = 
            np.adjacent(Coord2U::new2d(1, 1), true).unwrap().collect();
        let test = vec![
            GridAdjacent { index: 0, pos: Coord([0, 0]), dir: Intercardinal::Northwest, value: &1 }, 
            GridAdjacent { index: 1, pos: Coord([1, 0]), dir: Intercardinal::North, value: &2 }, 
            GridAdjacent { index: 2, pos: Coord([2, 0]), dir: Intercardinal::Northeast, value: &3 }, 
            GridAdjacent { index: 5, pos: Coord([2, 1]), dir: Intercardinal::East, value: &6 }, 
            GridAdjacent { index: 8, pos: Coord([2, 2]), dir: Intercardinal::Southeast, value: &9 }, 
            GridAdjacent { index: 7, pos: Coord([1, 2]), dir: Intercardinal::South, value: &8 }, 
            GridAdjacent { index: 6, pos: Coord([0, 2]), dir: Intercardinal::Southwest, value: &7 }, 
            GridAdjacent { index: 3, pos: Coord([0, 1]), dir: Intercardinal::West, value: &4 }
        ];
        assert_eq!(test, adjacent);
        ...
        let test = vec![
            GridAdjacent { index: 2, pos: Coord([2, 0]), dir: Intercardinal::North, value: &3 }, 
            GridAdjacent { index: 8, pos: Coord([2, 2]), dir: Intercardinal::South, value: &9 }, 
            GridAdjacent { index: 4, pos: Coord([1, 1]), dir: Intercardinal::West, value: &5 }
        ];
        assert_eq!(test, adjacent);
    }
}
