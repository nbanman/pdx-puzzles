...
impl<T, const N: usize> Grid<T, N> {
    pub fn new(vec: Vec<T>, dimensions: &[usize]) -> Result<Self, GridError> {
        ...
        if dimensions.len() == N {
            ...
            let dimension_chunk = dimensions.iter().fold(1, |acc, &n| acc * n);
            if vec.len() % dimension_chunk != 0 {
                return Err(GridError::UnevenDimensions);
            }
...
impl<T> Grid<T, 2> {
    ...
    where
        F: Fn(usize) -> T,
    {
        let vec = (0..width * height).map(|i| f(i)).collect();
        Grid {
            data: vec,
            dimensions: [width, height],
        }
...
impl<T> Grid<T, 3> {
    ...
    where
        F: Fn(usize) -> T,
    {
        let vec = (0..width * height * depth).map(|i| f(i)).collect();
        Grid {
            data: vec,
            dimensions: [width, height, depth],
        }
...
impl<T> TryFrom<Vec<Vec<T>>> for Grid<T, 2> {
    ...
    fn try_from(value: Vec<Vec<T>>) -> Result<Self, Self::Error> {
        ...
        let vec = value.into_iter().flatten().collect();
...
mod tests {
    ...
    #[test]
    fn try_from_uneven() {
        let new = vec![vec![1, 2, 3], vec![4, 5, 6, 7]];
        let new = Grid::try_from(new);
        assert_eq!(Err(GridError::UnevenDimensions), new);
    }
