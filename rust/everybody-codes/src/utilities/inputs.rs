...
pub fn get_story_inputs_provisional(year: u16, story: u8, day: u8) 
    -> (Option<String>, Option<String>, Option<String>) {
    let year = if year > 2000 { year - 2000 } else { year };
    (1..=3)
        .map(|part| get_story_input_provisional(year, story, day, part))
        .collect_tuple()
        .unwrap()
}

pub fn get_story_input_provisional(year: u16, story: u8, day: u8, quest: u8) -> Option<String> {
    let manifest_dir =
        std::env::var("CARGO_MANIFEST_DIR").expect("Failed to find manifest directory");
    let path = manifest_dir
        + &format!(
