...
fn circular_shots(input: Input, repeats: usize) -> usize {
    let ball_vec = get_balloons(input).repeat(repeats);
    let mut balloons = LinkedList::from_iter(ball_vec.clone());
    let ball_len = balloons.len();
    let mut cursor = balloons.cursor_mut();
    for _ in 0..ball_len / 2 {
        cursor.move_prev();
    }
    let mut kills = 0;
    let mut removals = 0;
    let mut offset = 0;
    let mut killed = vec![false; ball_len];
    while cursor.index
        .expect("cursor should never be none because kills are not removed") > kills + offset {
        if (ball_len - kills - removals) & 1 == 0 {
            if ball_vec[kills] == kills % 3 {
                let current = cursor.remove_current();
                removals += 1;
                killed
                println!("Kill {kills}: Removing {:?}, removals: {removals}", current);
            } else {
                println!("Kill {kills}: No match, moving");
                cursor.move_next()
            }

        } else {
            println!("Kill {kills}: Odd number remaining")
        }
        kills += 1;
    }
    kills
...
impl<'a, T> CursorMut<'a, T> {
    ...
    pub fn remove_current(&mut self) -> Option<T> {
        ...
        unsafe {
            let cur_node = Box::from_raw(cur_ptr.as_ptr());

            // Re-link neighbors
            match (cur_node.front, cur_node.back) {
                (Some(prev), Some(next)) => {
