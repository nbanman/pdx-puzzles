...
fn main() {
    ...
    println!("1. {} ({})", attack_ruins(&input1), stopwatch.lap().report());
    println!("2. {} ({})", attack_ruins(&input2), stopwatch.lap().report());
    println!("3. {} ({})", missile_defense(&input3), stopwatch.lap().report());
    ...
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        match self.pos.x().partial_cmp(&other.pos.x()) {
            Some(core::cmp::Ordering::Equal) => {
                other.pos.y().partial_cmp(&self.pos.y()) 
            },
            ord => ord,
        }
    }
...
fn attack_ruins(input: &str) -> usize {
    ...
    for target in towers {
        let ranking = catapults.iter()
            .filter_map(|catapult| {
                get_ranking(catapult, &target)
            })
            .next()
            .unwrap();
        score += ranking;
        ...
        for (x, &c) in line.as_bytes().iter().enumerate() {
            match c {
                ...
                b'T' => {
                    ...
                    let ruin = Ruins { target: Target::Block, pos };
                    tower.push(ruin);
                },
                b'H' => {
                    let pos = Pos::new2d(x as isize, y as isize);
                    let ruin = Ruins { target: Target::Rock, pos };
                    tower.push(ruin);
                }      
                _ => { },          
            }
