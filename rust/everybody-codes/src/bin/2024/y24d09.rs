...
use utilities::{parsing::{get_numbers::ContainsNumbers, try_get::TryGet}, structs::stopwatch::{ReportDuration, Stopwatch}};
...
fn part2(input: &str) -> usize {
    let stamps: Vec<usize> = [1, 3, 5, 10, 15, 16, 20, 24, 25, 30].into_iter().rev().collect();
    let brightnesses = get_brightnesses(input);
    let &brightest = brightnesses.iter().max().unwrap();
    let mut cache: Vec<Option<usize>> = vec![None; brightest + 1];
    solve(&brightnesses, &stamps, &mut cache)
...
fn part3(input: &str) -> usize {
    let stamps: Vec<usize> = [1, 3, 5, 10, 15, 16, 20, 24, 25, 30, 37, 38, 49, 50, 74, 75, 100, 101]
        .into_iter()
        .rev()
        .collect();
    let brightnesses = get_brightnesses(input);
    ...
    'outer: for (idx, &stamp) in stamps.iter().enumerate() {
        ...
        for n in (1..=beetles).rev() {  
            ...
            if let Some(&next_stamp) = stamps.try_get(idx + 1) {
                let next_stamp_beetles = ((remaining - n * stamp) as f32
                    / next_stamp as f32).ceil() as usize;
