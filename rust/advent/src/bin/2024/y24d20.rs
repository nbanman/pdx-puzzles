...
fn bfs(racetrack: &StrGrid, start: usize, end: usize, mut visited: Vec<usize>) -> Vec<usize> {
...
fn count_valid(input: &Input, pos: usize, end_coord: Pos, steps: usize) -> usize {
    if input.racetrack.get(pos) == Some(b'#') {
        return 0;
    }
    ...
    let (north, south) = [pos_coord.y(), end_coord.y()]
        .into_iter()
        .minmax()
        .into_option()
        .unwrap();
    ...
    for y in y_min..=y_max {
        let (west, east) = [pos_coord.x(), end_coord.x()]
            .into_iter()
            .minmax()
            .into_option()
            .unwrap();
        ...
        let x_min = max(1, max(pos_coord.x() - total_allowed, west - x_allowance));
        let x_max = min(
        ...
        for x in x_min..=x_max {
            ...
            if input.racetrack.get(re_pos) == Some(b'#') {
                continue;
            }
            let cheat_steps =
...
fn part1(input: &Input) -> Output {
    input
        ...
        .enumerate()
        .filter(|&(pos, &steps)| {
            input.racetrack.get(pos) == Some(b'#') && steps + input.from_end[pos] <= input.threshold
        })
        .count()
}
