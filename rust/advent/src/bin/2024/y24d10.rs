...
fn part1(topo_map: Input) -> Output {
    solve(topo_map, false)
}
fn part2(topo_map: Input) -> Output {
    solve(topo_map, true)
}
...
fn solve(topo_map: Input, distinct_paths: bool) -> Output {
    ...
    topo_map
        ...
        .map(|(trailhead, _)| paths(trailhead, topo_map.as_bytes(), width, distinct_paths))
        .sum()
}
...
fn paths(trailhead: usize, topo_map: &[u8], width: isize, distinct_paths: bool) -> usize {
    ...
    while let Some(state) = q.pop() {
        ...
        for neighbor in hike_path(state, topo_map, width) {
            if !distinct_paths {
                let visited = visited.as_mut().unwrap().get_mut(neighbor.pos).unwrap();
                if *visited {
                    continue;
                } else {
                    *visited = true;
...
fn hike_path(state: State, topo_map: &[u8], width: isize) -> impl Iterator<Item = State> + use<'_> {
    [-width, 1, width, -1]
        .into_iter()
