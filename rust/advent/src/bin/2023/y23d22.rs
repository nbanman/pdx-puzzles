...
impl Brick {
    fn new(from: Pos, to: Pos) -> Self {
        let (min_x, max_x) = [from.x(), to.x()]
            .into_iter()
            .minmax()
            .into_option()
            .unwrap();
        let (min_y, max_y) = [from.y(), to.y()]
            .into_iter()
            .minmax()
            .into_option()
            .unwrap();
        let (min_z, max_z) = [from.z(), to.z()]
            .into_iter()
            .minmax()
            .into_option()
            .unwrap();
...
impl BrickHouse {
    fn new(dimensions: Pos, bricks: &[Brick]) -> Self {
        let mut space: Vec<Option<Brick>> = vec![
            None;
        ...
        self.bricks
            ...
            .filter(|&brick| {
                if let Some(rested_bricks) = self.rested_on.get(brick) {
                    rested_bricks
                        .iter()
                        .all(|rested_brick| self.resting_on[rested_brick].len() > 1)
                } else {
                    true
                }
            })
